...
class DynamicRouterControlEndpointTest {
    ...
    void setup() {
        ...
        endpoint = new DynamicRouterControlEndpoint(CONTROL_ENDPOINT_URI, component, "subscribe", configuration);
        producerFactory = new DynamicRouterControlProducerFactory() {
            @Override
            public DynamicRouterControlProducer getInstance(
                    DynamicRouterControlEndpoint endpoint, DynamicRouterComponent routerComponent,
                    DynamicRouterControlConfiguration configuration) {
                return producer;
            }
        };
        endpoint.setControlProducerFactorySupplier(() -> producerFactory);
